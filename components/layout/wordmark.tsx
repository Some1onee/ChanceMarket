...
export function Wordmark({ className, asLink = true }: { className?: string; asLink?: boolean }) {
  const mark = (
    <span
      className={cn(
        "inline-flex items-baseline font-display text-xl font-bold tracking-tight select-none",
        className,
      )}
    >
      <span
        aria-hidden
        className="mr-1.5 inline-block size-2.5 translate-y-[-1px] rounded-full bg-accent"
      />
